...
export enum WH_RecycleView_Type {
    /**
     * @zh 水平布局。
     */
    HORIZONTAL = 1,
...
export enum WH_RecycleView__AxisDirection {
    /**
     * @zh 进行水平方向布局。
     */
    HORIZONTAL = 0,
...
export default class WHRecycleView extends cc.Component {
    @property({
        type: cc.Node,
        tooltip: '页面适配器节点',
    })
    ...
    public get type(): WH_RecycleView_Type {
        return this._layoutType;
    }
    public set type(v: WH_RecycleView_Type) {
        this._layoutType = v;
    }
    ...
    public get startAxis(): WH_RecycleView__AxisDirection {
        return this._startAxis;
    }
    public set startAxis(v: WH_RecycleView__AxisDirection) {
        this._startAxis = v;
    }
    ...
    protected _layoutType = WH_RecycleView_Type.VERTICAL;

    @property({
        serializable: true,
    })
    protected _startAxis = WH_RecycleView__AxisDirection.VERTICAL;
    @property({
        serializable: true,
    })
    ...
    protected _adapter: WHRecycleAdapter<WHRecycleHolder> = null;
    public get adapter(): WHRecycleAdapter<WHRecycleHolder> {
    ...
    /**当前现实在屏幕里的item */
    protected _childrens: Array<WHRecycleHolder> = new Array();
    ...
    */
    ...
    protected _createItem(index): WHRecycleHolder {
        let item: WHRecycleHolder;
    ...
    protected _inScreenByItem(item: WHRecycleHolder) {
        if (!item) {
            return;
        }
    ...
    protected _getPositionInView(item: WHRecycleHolder): cc.Vec3 {
    ...
    //将节点放入内存池并删除
    protected _removeItem(item: WHRecycleHolder) {
        if (!item) {
            return;
        }
    ...
    onDestroy() {
        ...
        for (let key in this._pool) {
            const items: Array<WHRecycleHolder> = this._pool[key];
            items.forEach((v) => {
                v = null;
            })
            items.length = 0;
...
export abstract class WHRecycleHolder extends cc.Component {
    //节点类型
    type: number;
    //节点标识
    tag: number;
...
export abstract class WHRecycleAdapter<VH extends WHRecycleHolder> extends cc.Component {
    //获取节点数量
