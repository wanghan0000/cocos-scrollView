...
export default class SaasTabView extends cc.Component {
    ...
    private createItems(value: number) {
        ...
        if(this.zIndexDir){
            this.scrollView.content.width = value * 142 + (value - 1) * 20 + 40;
        }
        ...
        for (let i = 0; i < total; ++i) {
            ...
            if(this.zIndexDir){
                child.zIndex = total - i;
               
            }
            this.scrollView.content.addChild(child);
            this.childrens.push(child);
            if(this.zIndexDir){
                child.x = i * 142 + 20 + i * 20 + 71;
                child.y = 0;
            }
        }
        //通知所有节点刷新
    ...
    onEnable(){
        this.childrens.forEach((node) => {
            node.on(cc.Node.EventType.TOUCH_END, this.itemClickCallback, this);
        })
    }
